...
class StreamingFrameRenderer
{
public:
    using GstEventCallback = std::function<void(winrt::MediaFramework::GstEventArgs const&)>;

    StreamingFrameRenderer(winrt::Microsoft::UI::Dispatching::DispatcherQueue const& uiDispatcherQueue,
...
private:
    ...
    void RenderLoop();
    void RenderSwapChainPanel(ComPtr<ID3D11Texture2D> srcTexture);
